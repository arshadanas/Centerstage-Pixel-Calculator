...
import static pixelcalc.algorithm.Pixel.Color.*;
...
/**
 * Implementation of Arshad Anas's CENTERSTAGE Optimal Pixel Placement Algorithm
 * 
 * @author Arshad Anas
 */
public final class PlacementCalculator {
    ...
    private final ArrayList<Pixel> optimalPlacements = new ArrayList<>(),
            colorsToGetSPixels = new ArrayList<>();

    private ArrayList<Pixel> setLineSPixels;
    private final boolean auton;
    // private final boolean specifyColors = false;
    ...
    /**
     * @return Whether the provided {@link Pixel} is part of a special case of
     *         mosaic case priorities
     *         present in the perfect backdrop layout, where the up case is
     *         preferred over right or left to allow
    ...
    private boolean upMosaicIsHelpful(Pixel pixel) {
        ...
        if (!(x == 3 && (y == 3 || y == 9)))
            return false;
        if (y == 9)
            return upMosaicIsHelpful(new Pixel(x, 3, pixel.color));
        ...
        for (Pixel p : shouldBeColored)
            if (!(p.color == EMPTY || p.color.isColored()))
                return false;
        ...
        for (Pixel p : shouldBeWhite)
            if (!(p.color == EMPTY || p.color == Pixel.Color.WHITE))
                return false;
    ...
    /**
     * @return The three possible mosaic cases for the provided {@link Pixel}'s
     *         coordinates <br>
    ...
    private Pixel[][] getPossibleMosaics(Pixel pixel) {
        ...
        if (x == 1)
            return new Pixel[][] { left, up, right };
        if (x == 6)
            return new Pixel[][] { right, up, left };
        if (upMosaicIsHelpful(pixel))
            return new Pixel[][] { up, right, left };
        if (x == 5 || x == 3)
            return new Pixel[][] { right, left, up };
        return new Pixel[][] { left, right, up };
    }

    /**
     * Subtract values from {@link #colorsLeft} based on the colors present on
     * {@link #backdrop}
     */
    private void countColorsLeft() {
        for (Pixel[] row : backdrop.slots)
            for (Pixel pixel : row)
                if (pixel.color.isColored()) {
    ...
    /**
     * Iterate through {@link #backdrop}, detect and mark valid mosaics, mark
     * invalid mosaics
     * by {@link Pixel}.mosaic attribute and add any {@link Pixel}s required to
     * complete a mosaic
     * to {@link #optimalPlacements} and {@link #colorsToGetSPixels} <br>
     * After that, any required {@link Pixel} with ANYCOLOR as its color is given a
     * specific color
    ...
    private void scanForMosaics() {
        ...
        for (int y = 0; y < backdrop.slots.length; y++)
            for (int x : iterationXs(y)) {
                ...
                for (Pixel[] pMosaic : possibleMosaics) {
                    ...
                    boolean diffColors = pMosaic[0].color != pMosaic[1].color && pMosaic[1].color != pMosaic[2].color
                            && pMosaic[0].color != pMosaic[2].color;
                ...
                if (isMosaic) {
                    ...
                    if (pixels[0].mosaic.color != Pixel.Color.INVALID)
                        backdrop.mosaicCount++;
                    continue;
                }

                pMosaics: for (Pixel[] pMosaic : possibleMosaics) {
                    for (Pixel a : pMosaic)
                        if (a.color == Pixel.Color.WHITE)
                            continue pMosaics;
                    for (Pixel a : pMosaic)
                        if (a.color.isColored() || a.color == EMPTY)
                            a.mosaic = pixel;

                    if (!touchingAdjacentMosaic(pMosaic[0], false) &&
        ...
        for (Pixel p : colorsToGetSPixels)
            optimalPlacements.addAll(getSupportPixels(p));
        removeDuplicates(optimalPlacements);
    ...
    private Pixel.Color getRemainingColor(Pixel.Color c1, Pixel.Color c2) {
        ...
        if (c1 == c2)
            return c1;
        ArrayList<Pixel.Color> colors = new ArrayList<>(
                Arrays.asList(Pixel.Color.GREEN, Pixel.Color.PURPLE, Pixel.Color.YELLOW));
        colors.remove(c1);
        colors.remove(c2);
        ...
        for (Pixel neighbor : backdrop.getNeighbors(pixel)) {
            if (neighbor.mosaic != pixel.mosaic) {
                ...
                if (includeEmpties && neighbor.color == EMPTY && neighbor.mosaic != null) {
                    ...
                    pMosaics: for (Pixel[] pMosaic : pMosaics) {
                        for (Pixel p : pMosaic)
                            if (p.mosaic == null)
                                continue pMosaics;
                    ...
                    if (Backdrop.allTrue(pMosaicsTouchingPixel))
                        return true;
    ...
    private void removeOverridingPixels(ArrayList<Pixel> pixels) {
        ...
        pixels.clear();
        for (Pixel pixel : pixelsCopy)
            if (backdrop.get(pixel).color == EMPTY)
                pixels.add(pixel);
    }
    ...
    private Pixel getSetLineGoal() {
        ...
        int setY = highestY >= 5 ? 8 : highestY >= 2 ? 5 : 2;

        int leastSPixels = 100;
        Pixel bestSetPixel = backdrop.get(6, 8);

        setPixels: for (int x : iterationXs(setY)) {
            Pixel pixel = backdrop.get(x, setY);
            if (pixel.color == Pixel.Color.INVALID)
                continue;
            ArrayList<Pixel> sPixels = getSupportPixels(pixel);
            if (auton)
                for (Pixel p : sPixels) {
                    if (p.isIn(colorsToGetSPixels))
                        continue setPixels;
                }
    ...
    private void scanForSetLinePixels() {
        ...
        if (setLineGoal.y <= 8)
            optimalPlacements.addAll(setLineSPixels);
        removeUnsupportedPixels(optimalPlacements);
    }
    ...
    private Pixel getSafePixel(Pixel pixel) {
        Pixel p1 = new Pixel(pixel,
                touchingAdjacentColor(pixel) || touchingAdjacentMosaic(pixel, true) || noSpaceForMosaics(pixel) || auton
                        || !isMosaicPossible() ? Pixel.Color.WHITE :
