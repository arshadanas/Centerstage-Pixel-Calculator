...
public final class Backdrop {
    ...
    public boolean equals(Object other) {
        if (!(other instanceof Backdrop otherBackdrop))
            return false;
        for (Pixel[] row : slots)
            for (Pixel pixel : row) {
                if (otherBackdrop.get(pixel).color != pixel.color)
                    return false;
            }
        return true;
    }
     ...
     * If provided a {@link Pixel} of {@link Pixel.Color} ANY or ANYCOLOR, this
     * method will randomly choose a specified version of the provided
     * {@link Pixel.Color}
     * 
        ...
        switch (pixel.color) {
            ...
            default:
                ...
                if (coordsInRange(x, y) && !(y % 2 == 0 && x == 0)) {
                    slots[y][x] = pixel.recommended != EMPTY && random() >= 0.5 ? new Pixel(pixel, pixel.recommended)
                            : pixel;
                }
        }
        return this;
    ...
    /**
     * @return The {@link Pixel} found at the provided x and y coordinates, if
     *         {@link #coordsInRange}
     */
    ...
    /**
     * @return The {@link Pixel} found at the corresponding location of the provided
     *         {@link Pixel}, if {@link #coordsInRange}
     */
    ...
    /**
     * @return Whether the provided x, y location is within the bounds of this
     *         {@link Backdrop} object
     */
    ...
    /**
     * @return A multi-line {@link String} representation of the {@link Pixel}
     *         contents of this {@link Backdrop} object
     */
    public String toString() {
        ...
        for (int y = ROWS - 1; y >= 0; y--) {
            ...
            for (int x = 0; x < COLUMNS; x++) {
                ...
                if (!(y % 2 == 0 && x == 0) || printRectangular)
                    backdrop.append(color);
        ...
        for (int x = 0; x < COLUMNS; x++)
            backdrop.append(x).append(x == COLUMNS - 1 ? "" : spacer);
        return backdrop.toString();
    }

    /**
     * Output the result of {@link #toString()}, {@link #mosaicCount}, and points
     * scored to the main text output stream
     */
    public void print() {
     ...
     * @return The 6 directly adjacent neighbor {@link Pixel} objects of the
     *         provided {@link Pixel}
     */
    ...
    /**
     * @return Whether the two provided {@link Pixel}s are directly adjacent as per
     *         {@link #getNeighbors}
     */
    ...
    /**
     * @return Whether the provided {@link Pixel} is supported by two other
     *         {@link Pixel}s below it in hexagonal grid space
     */
    public boolean isSupported(Pixel pixel) {
        return get(pixel.x, pixel.y - 1).color != EMPTY
                && get(pixel.x - 1 + 2 * (pixel.y % 2), pixel.y - 1).color != EMPTY;
    }
    ...
    int getHighestPixelY() {
        ...
        for (Pixel[] row : slots)
            for (Pixel p : row) {
                if (!(p.color == EMPTY) && p.color != INVALID)
                    highestY = p.y;
            }
        return highestY;
    }
    ...
    private int getPixelCount() {
        ...
        for (Pixel[] row : slots) {
            for (Pixel pixel : row) {
                if (!(pixel.color == EMPTY || pixel.color == INVALID))
                    pixelCount++;
            }
        }
        return pixelCount;
    ...
    /**
     * @return The number of "set lines" reached by the highest pixel on this
     *         {@link Backdrop}
     */
